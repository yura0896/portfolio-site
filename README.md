...
```css
--bg:            #ffffff;   /* 背景 */
--bg-sub:        #e3f4fd;   /* セクション背景（水着イラストのプール色を薄めた水色） */
--fg:            #12344d;   /* 文字（線画に寄せた濃い紺） */
--sticker:       #ffffff;   /* 画像まわりの白フチ */

--accent:        #90c0c0;   /* くすんだ青緑（ブランドカラー）。ロゴ周りなどの塗り */
--accent-text:   #24686a;   /* 同色相の濃色。リンク・見出しの文字 */
--accent-fg:     #123030;   /* --accent の上に乗る文字 */
--accent-2:      #f7bf94;   /* 温かいオレンジ。「要相談」の丸など */
--accent-2-text: #5a2a08;   /* --accent-2 の上に乗る文字 */

--pink:          #ff8fb5;   /* ボタン・選択中のナビ・番号の丸。上に乗る文字は --pink-fg */
--sky:           #4aa8d8;   /* 枠線・区切り線 */
--tile:          #9ad5f2;   /* ヒーローのタイル地（--tile-line が目地） */
```

`--accent` の `#90c0c0` は、X（@Yukishiro_Yu_Ki）のヘッダー画像から実際に抽出した色です。
画像全体の 85.9% をこの色が占めていました。2026-09-20 に、作品（水着イラストのプール、
ピンク、白フチのステッカー）に寄せるため、`--pink` / `--sky` / `--tile` を足して
全体を「アクア・タイル」のデザインに変えました（ブランドカラーの青緑は差し色として残しています）。
白フチ（`--sticker`）は、カード・詳細画像・プロフィール写真・ヒーローのキャラに適用しています。
...
|---|---|---|
| 本文 | 12.9:1 | 14.4:1 |
| キャッチコピー（タイル地の上） | 8.1:1 | 8.9:1 |
| 肩書き（白い札の上） | 12.9:1 | 10.8:1 |
| 補足テキスト（水色セクションの上） | 5.0:1 | 5.9:1 |
| 主ボタン（ピンクの上） | 6.1:1 | 6.1:1 |
| 副ボタン | 12.9:1 | 12.9:1 |
| SNSリンク | 7.3:1 | 7.3:1 |
| ANIMATION / ILLUSTRATION タグ | 12.9:1 | 12.9:1 |
| 受付状況の「○」の丸 | 6.3:1 | 6.3:1 |

ヒーローの名前「ちばゆうき」は白文字に濃紺の縁取り（`text-shadow`）で読ませる大きな文字なので、
上の表とは別扱いです（縁取りが無いと白×水色は 1.6:1 で読めません）。

ダークテーマ（「夜のプール」：紺色の地に目地だけが光る）は `:root[data-theme="dark"]` と
`@media (prefers-color-scheme: dark)` の 2箇所に同じ値を書いています。
**片方だけ直さないよう注意**してください。
...
| 要素 | 変数・箇所 | 効果 |
|---|---|---|
| 角の丸み | `--radius` (26px) | 大きいほど柔らかい。小さくすると落ち着く |
| ボタンの形 | `--radius-pill` | 完全なピル型。ふちは白フチ+濃紺の二重線（`.btn`） |
| ヒーローの地 | `.hero` の `background-image` / `--tile` `--tile-line` | プールのタイル柄。`background-size` で目地の大きさを調整 |
| ヒーローのキャラ | `.hero__chara` | `drop-shadow` を4方向に重ねた白フチ + ふわふわ動くアニメ（`@keyframes bob`） |
| セクションの段差 | `.section--sub` の `border-radius` | 上端44pxの丸み。紙を重ねたような見え方 |
