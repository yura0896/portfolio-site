...
--bg-sub:        #eff8fa;   /* セクション背景（淡水色） */
--fg:            #3a3742;   /* 文字（真っ黒を避けた濃色） */

--accent:        #8ed3e0;   /* 淡水色。ボタンの塗り・枠線 */
--accent-text:   #17788d;   /* 同色相の濃色。リンク・見出しの文字 */
--accent-fg:     #0d3b44;   /* --accent の上に乗る文字 */
--accent-2:      #fbc9d8;   /* ピンク。イラスト作品のタグ、番号の丸 */
--accent-2-text: #8a2d4f;   /* --accent-2 の上に乗る文字 */
```

### 差し色が「塗り用」と「文字用」に分かれている理由

淡い色は塗りには使えても文字には使えません。淡水色をそのまま文字色にすると、
...
|---|---|---|
| 本文 | 11.5:1 | 15.4:1 |
| 肩書き・リンク | 5.0:1 | 11.0:1 |
| 補足テキスト | 4.8:1 | 5.6:1 |
| ボタンの文字 | 7.3:1 | 6.4:1 |
| ANIMATION タグ | 7.3:1 | 6.4:1 |
| ILLUSTRATION タグ | 5.6:1 | 7.8:1 |
...
| ヒーローの光 | `.hero::before` / `::after` | 水色とピンクのぼかし円。`opacity` で強さを調整 |
